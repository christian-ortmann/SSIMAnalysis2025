...
mine7cont$FP <- calculate_FP(mine7cont$tempC, mine7cont$time)

```

```{r standardize data for lasso logit}

noTime <- subset(mcont, select = -c(datetime, rockfall))
...
# Create the combined plot
irr7_combined <- ggplot(m8cont, aes(x = datetime)) + 
  # Temperature plot
  geom_line(aes(datetime, na.locf(rollapply(irradiance, width = 24, FUN = mean, fill = NA, align = 'right', partial = TRUE)), color = "Irradiance 24hr")) + 
  # Rockfall data scaled to match temperature range
...
# Create the combined plot
insol7_combined <- ggplot(m8cont, aes(x = datetime)) + 
  # Temperature plot
...
comb <- (t7_combined + p7_combined) / (pi7_combined + wind7_combined) / (irr7_combined + insol7_combined) + 

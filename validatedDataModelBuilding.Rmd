...
m7Data <- readRDS(here("mine7StandardizedDataForML.rds")) 
head(m7Data) 

m7UnStandardData <- readRDS(here("mine7DataForML.rds")) 
head(m7Data) 
...
m7WOE <- subset(m7UnStandardData, select = -c(datetime))
...
breaks_list <- list(intensity = seq(min(df$intensity), max(df$intensity), by = ((max(df$intensity)-min(df$intensity))/5)), 
                    ...
                    tempC = seq(min(df$tempC), max(df$tempC), by = ((max(df$tempC)-min(df$tempC))/5)), 
                    irradiance = seq(min(df$irradiance), max(df$irradiance), by = ((max(df$irradiance)-min(df$irradiance))/5)),
                    solarInsol24 = seq(min(df$solarInsol24), max(df$solarInsol24), by = ((max(df$solarInsol24)-min(df$solarInsol24))/5)),
                    windMS = seq(min(df$windMS), max(df$windMS), by = ((max(df$windMS)-min(df$windMS))/5)), 
                    ...
                    fp = seq(min(df$FP), max(df$FP), by = ((max(df$FP)-min(df$FP))/5)))
...
#this is not actually used and just plots the predictions
# 
# 

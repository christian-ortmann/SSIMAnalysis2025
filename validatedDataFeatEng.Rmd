...
noTime <- subset(m7cont, select = -c(datetime, rockfall))
...
max_rockfalls <- 10  # Set max rockfall count to 15
...
t7_combined <- ggplot(m7cont, aes(x = datetime)) + 
  ...
  # geom_area(data = rockfallsDailyCount, aes(x = as.POSIXct(avg_time), y = rockfall * (max_temp / max_rockfalls),color = ), alpha = 0.5, fill = "lightgray")
geom_area(data = rockfallsDailyCount, 
            aes(x = as.POSIXct(avg_time), 
  ...
  scale_y_continuous(limits = c(min_temp, max_temp),  # Fix temperature axis limits
                     sec.axis = sec_axis(~ (. - min_temp) * (max_rockfalls / (max_temp - min_temp)), name = "Rockfall Count", breaks = seq(0, 10, 2)), expand = expansion(mult = c(0, 0.05))) + 
